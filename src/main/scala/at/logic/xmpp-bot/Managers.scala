...
case class XMPPManager() {
  ...
  //Login
  private def login(username: String, password: String, server: String): XMPPConnection = {
    val connection = new XMPPConnection(connectionConfig(server))
    connection.connect()
  ...
  def setup(username: String, password: String, server: String) = {
//    println("Username: ")
//    val username = read()
//    println("Password: ")
...
case class XMPPAgentManager(username: String, password: String, server: String, agents: ActorRef*) extends AgentManager(agents : _*) with ChatManagerListener with MessageListener {
  val connectionManager = XMPPManager()
  //Setup Connection
  
  connectionManager.setup(username, password, server)
  //Start Chat
